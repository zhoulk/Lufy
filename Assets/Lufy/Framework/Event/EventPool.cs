...
namespace LF.Event
{
    internal sealed partial class EventPool<T> where T : GameEventArgs
    {
        ...
        {
            LinkedList<EventHandler<T>> range = default(LinkedList<EventHandler<T>>);
        ...
        public bool Check(int id, EventHandler<T> handler)
        {
            if (handler == null)
            {
                throw new LufyException("Event handler is invalid.");
            }
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            if (!m_EventHandlers.ContainsKey(id))
            {
                ...
                LinkedList<EventHandler<T>> range = new LinkedList<EventHandler<T>>();
                range.AddLast(handler);
                m_EventHandlers.Add(id, range);
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowMultiHandler) == 0)
            {
                throw new LufyException(Utility.Text.Format("Event '{0}' not allow multi handler.", id.ToString()));
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowDuplicateHandler) == 0 && Check(id, handler))
            {
                throw new LufyException(Utility.Text.Format("Event '{0}' not allow duplicate handler.", id.ToString()));
            }
            else
            {
                LinkedList<EventHandler<T>> range = default(LinkedList<EventHandler<T>>);
        ...
        public void Unsubscribe(int id, EventHandler<T> handler)
        {
            ...
            if (m_EventHandlers.TryGetValue(id, out range))
            {
                range.Remove(handler);
            }
            else
            {
                throw new LufyException(Utility.Text.Format("Event '{0}' not exists specified handler.", id.ToString()));
            }
        }
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            {
                throw new LufyException(Utility.Text.Format("Event '{0}' not allow no handler.", e.Id.ToString()));
            }
        }
    }
}
